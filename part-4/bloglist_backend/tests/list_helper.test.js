...
describe('most blogs', () => {
  ...
  test('when list has only one blog, equals to that', () => {
    expect(listHelper.mostBlogs([helper.testBlogs[0]])).toEqual({ author: 'Vsk', blogs: 1 })
  })

  test('from a bigger list is selected right', () => {
    expect(listHelper.mostBlogs(helper.testBlogs)).toEqual({ author: 'Krish', blogs: 3 })
  })
})
...
describe('most likes', () => {
  ...
  test('when list has only one blog, equals to that', () => {
    expect(listHelper.mostLikes([helper.testBlogs[2]])).toEqual({ author: 'Srikrishna', likes: 12 })
  })

  test('from a bigger list is selected right', () => {
    expect(listHelper.mostLikes(helper.testBlogs)).toEqual({ author: 'Vnr', likes: 17 })
  })
})